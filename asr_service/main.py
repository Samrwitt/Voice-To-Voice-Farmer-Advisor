...
from schemas import ASRResponse, FileTranscribeRequest
from engine import ASREngine
from audio_utils import save_upload_file, prepare_audio_for_asr
from config import SHARED_UTTERANCES_DIR


app = FastAPI(
    title="Amharic ASR Service",
    description="Fine-tuned Whisper CT2 ASR service with Amharic post-processing.",
    version="0.1.0",
)
...
def health():
    return {
        "status": "ok" if asr_engine is not None else "degraded",
        "service": "asr_service",
...
async def transcribe(
    ...
    if asr_engine is None:
        raise HTTPException(status_code=503, detail="ASR engine is not initialized.")
