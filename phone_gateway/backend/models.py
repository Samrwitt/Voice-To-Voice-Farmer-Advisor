from sqlalchemy import Column, String, DateTime, Float, ForeignKey, Table, Integer
...
class Caller(Base):
    ...
    profile = relationship("FarmerProfile", back_populates="caller", uselist=False)
...
class Crop(Base):
    ...
    farmers = relationship("FarmerProfile", secondary=farmer_crop_association, back_populates="crops")
...
class AudioFile(Base):
    ...
    id = Column(String, primary_key=True, index=True)
    session_id = Column(String, ForeignKey("call_sessions.session_id"))
    file_path = Column(String, nullable=False)
    type = Column(String, nullable=False) # e.g., 'caller', 'mixed', 'agent'
    created_at = Column(DateTime, default=datetime.utcnow)
...
class Transcript(Base):
    ...
    id = Column(String, primary_key=True, index=True)
    asr_job_id = Column(String, ForeignKey("asr_jobs.id"), unique=True)
